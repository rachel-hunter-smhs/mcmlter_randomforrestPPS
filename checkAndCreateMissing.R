...
ensure_reg <- function(ds, target, reg, df, tag, dirp) {
  obs   <- df[[target]]
  preds <- predict(reg, df %>% select(-total, -total_live, -total_dead, everything()))
  res   <- obs - preds
...
ensure_clf <- function(ds, target, clf, df, tag, dirp, p80) {
  dfp <- df %>%
    filter(!is.na(.data[[target]])) %>%                                   # drop rows with NA target
    mutate(class_obs = factor(if_else(.data[[target]] >= p80, "High", "Low"))) %>%
    select(-total, -total_live, -total_dead, everything())
  
  probs_all <- predict(clf, dfp, type = "prob")[, "High"]                 # may contain NA
  keep_idx  <- !is.na(probs_all)                                          # keep only rows with valid probs
...
for (ds_dir in dir_ls(reg_root, recurse = FALSE, type = "directory")) {
  ...
  train_df <- read_csv(path(processed_dir, paste0(ds, "_train.csv")), show_col_types = FALSE)
  valid_df <- read_csv(path(processed_dir, paste0(ds, "_validation.csv")), show_col_types = FALSE)
  test_df  <- read_csv(path(processed_dir, paste0(ds, "_test.csv")),  show_col_types = FALSE)

  for (reg_dir in dir_ls(ds_dir, recurse = FALSE, type = "directory")) {
    target <- path_file(reg_dir)
    model_path <- path(reg_dir, "model.rds")
    if (!file_exists(model_path)) next
    ...
    if (!file_exists(path(reg_dir, "oob_error.png")) && !is.null(reg$mse)) {
...
for (ds_dir in dir_ls(class_root, recurse = FALSE, type = "directory")) {
  ...
  for (clf_dir in dir_ls(ds_dir, recurse = FALSE, type = "directory")) {
    ...
    if (!file_exists(model_path)) model_path <- path(clf_dir, "model.rds")
    if (!file_exists(model_path)) next

    clf <- readRDS(model_path)
    p80 <- quantile(train_df[[target]], .8, na.rm = TRUE)

    ensure_clf(ds, target, clf, valid_df, "validation", clf_dir, p80)
    ensure_clf(ds, target, clf, test_df,  "test",        clf_dir, p80)

    if (!file_exists(path(clf_dir, "oob_error.png")) && !is.null(clf$err.rate[, "OOB"])) {
